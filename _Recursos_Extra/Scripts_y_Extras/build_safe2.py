...
# Let's find exactly MS_Origen node
ms_origen_idx = content.find('name="MS_Origen"')
if ms_origen_idx == -1:
    print("Could not find MS_Origen")
    exit(1)

# Find the start of the tag
start_idx = content.rfind('<point', 0, ms_origen_idx)

# We need to find the last sleeve node to replace up to.
# Since we git checkout'ed, the file has the old sleeve.
# Let's find the piece "Manga_Int" which is defined later.
# We want to replace everything from start_idx up to the first </nodes> AFTER start_idx
end_nodes_idx = content.find('</nodes>', start_idx)

# But wait, we must not replace nodes that are NOT part of the sleeve!
# Is there anything else between MS_Origen and </nodes>?
# Let's just find the last node that contains MS_ or Manga_ in its name or something.
# The safest way is to just grab the substring, and check if any non-sleeve nodes exist.
sleeve_section = content[start_idx:end_nodes_idx]
lines = sleeve_section.splitlines()
last_sleeve_line_idx = -1
for i, line in enumerate(lines):
    if 'id=' in line:
        last_sleeve_line_idx = i

# In the old file, the last sleeve node is probably around 12022 or something.
# Let's just replace everything from start_idx to end_nodes_idx, since MS_Origen is the last root node
# appended to the block. The Bodice nodes are all above MS_Origen.

new_sleeve_nodes = """<point id="12000" mx="0.1" my="0.1" name="MS_Origen" showPointName="true" type="single" x="150" y="0"/>
            ...
            <point angle="180" basePoint="12002" id="30003" length="Line_MS_Codo_Izq_MS_Codo_Der / 4" mx="0.1" my="0.1" name="Fold_Frente_Codo" showPointName="false" type="endLine"/>
            <point angle="0" basePoint="12002" id="30004" length="Line_MS_Codo_Izq_MS_Codo_Der / 4" mx="0.1" my="0.1" name="Fold_Espalda_Codo" showPointName="false" type="endLine"/>
            <point angle="180" basePoint="12001" id="30005" length="Line_MS_Puno_Izq_MS_Puno_Der / 4" mx="0.1" my="0.1" name="Fold_Frente_Puno" showPointName="false" type="endLine"/>
            <point angle="0" basePoint="12001" id="30006" length="Line_MS_Puno_Izq_MS_Puno_Der / 4" mx="0.1" my="0.1" name="Fold_Espalda_Puno" showPointName="false" type="endLine"/>
