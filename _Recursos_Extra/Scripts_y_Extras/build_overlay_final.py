...
# MS_Origen point
start_idx = content.find('name="MS_Origen"')
start_idx = content.rfind('<point ', 0, start_idx)

# Ext_MS_Puno_Bajera_Der point
last_point_idx = content.find('name="Ext_MS_Puno_Bajera_Der"')
if last_point_idx == -1:
    print("Could not find Ext_MS_Puno_Bajera_Der")
    exit(1)

last_line_idx = content.find('firstPoint="89504"', last_point_idx)
if last_line_idx == -1:
    replace_end_idx = content.find('/>', last_point_idx) + 2
else:
    replace_end_idx = content.find('/>', last_line_idx) + 2
...
new_sleeve_nodes = """<point id="12000" mx="0.1" my="0.1" name="MS_Origen" showPointName="true" type="single" x="150" y="0"/>
            ...
            <line firstPoint="12004" id="30061" lineColor="black" lineType="none" secondPoint="12000"/>
            <line firstPoint="12000" id="30062" lineColor="black" lineType="none" secondPoint="12005"/>
            ...
            <point angle="90" basePoint="30002" curve="12021" id="30032" lineColor="black" lineType="none" mx="0.1" my="0.1" name="Copa_Espalda_Pico" showPointName="true" type="curveIntersectAxis"/>
...
new_content = content[:start_idx] + new_sleeve_nodes + content[replace_end_idx:]

with open(file_path, "w", encoding="utf-8") as f:
    f.write(new_content)
print("Sleeve nodes perfectly replaced with correct formulas!")
